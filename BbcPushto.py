...
pushto = []
url = "http://feeds.bbci.co.uk/pashto/rss.xml"
pushto.append(url)


junk = "دا له نورو سره شریک کړئبریښنالیکفېسبوکMessengerMessengerټوېټرګوګل پلسWhatsAppدا لېنک کاپي کړئدا د نورو وېبپاڼو لېنکونه دي چې په نوې کړکۍ کې پرانستل کېږيد"
        ...
        if pt == "True":
            ...
            try:
                try:
                    ...
                    body = p_content
                    body = body.replace("دا له نورو سره شریک کړئبریښنالیکفېسبوکMessengerMessengerټوېټرګوګل پلسWhatsAppدا لېنک کاپي کړئدا د نورو وېبپاڼو لېنکونه دي چې په نوې کړکۍ کې پرانستل کېږيد","")
                    print(body)
                except:
                    p_content = ''
                ...
                if title is '':
                    ...
                    try:
                        collection1.insert([{"Type": "Predefined List", "Category": "International", "Language": "Pushto",
                                             "published Time": pubTime, "Source": "BBC Pushto", "title": title, "body": body,
                                             "_id": lnk}])

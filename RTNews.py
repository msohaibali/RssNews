...
rt.append('https://www.rt.com/rss/news/')
...
def NewsContent(lnk):
    ...
    for d4 in data:
        ...
        if ht == "True":
            ...
            try:
                curr_post = soup.find('h1', attrs={'class': 'article__heading'})

                title = curr_post.text
                title = title.replace("\n", "")
                title = title.replace("        ", "")
                title = title.replace("(VIDEO, PHOTO)", "")
                title = title.replace("(VIDEO)", "")
                title = title.replace("(PHOTO, VIDEO)", "")
                title = title.replace("(PHOTO)", "")
                title = title.replace("(PHOTOS, VIDEOS)", "")
                print(title)
            ...
            try:
                p_content = ''
                curr_post = soup.find('div', attrs={'class': 'article__summary summary '})
                curr_post2 = soup.find('div', attrs={'class': 'article__text text '})
                targ_p = curr_post2.findAll('p')
                p_content = p_content + curr_post.text
                for p in targ_p:
                ...
                body = body.replace("\n", "")
                body = body.replace("        ", "")
                body = body.replace("Think your friends would be interested? Share this story!", "")
                body = body.replace("If you like this story, share it with a friend!", "")
                body = body.replace("Like this story? Share it with a friend!", "")
                print(body)
                ...
                try:
                    ...
                    collection1.insert([{"Type": "Predefined List", "Category": "International", "Language": "English",
                                         "Source": "RT News", "title": title, "body": body, "_id": lnk,
